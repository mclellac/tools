...
# Contact:  careymclelland at gmail.com                                    #
# Twitter:  https://twitter.com/careymclelland                             #
# LinkedIn: https://www.linkedin.com/in/careymclelland/                    #
############################################################################
...
declare -A suidlibrary=(
	...
	[sqlite3]="Read the restricted file: LFILE=file_to_read; sqlite3 << EOF; CREATE TABLE t(line TEXT); .import \$LFILE t; SELECT * FROM t; EOF"
...
do
	suidcommand=$(echo $suidexploitable | awk -F "/" '{print $NF}'); # clear the path
	## check for matches
    for suidSelect in "${rootsuidlist[@]}"; do
