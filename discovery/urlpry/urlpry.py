    ...
    try:
        ...
        return {
            ...
            "HTML Info": {
                ...
                "Meta": {
                    meta.attrs.get("name", "unknown"): meta.attrs.get("content", "") for meta in soup.find_all("meta")
                },
...
def detect_social_media_links(url: str) -> Union[List[str], Dict[str, str]]:
    try:
        ...
        return [
            ...
            if any(social in a["href"] for social in ["facebook.com", "twitter.com", "linkedin.com"])
        ]
    except requests.RequestException as e:
        return {"Error": str(e)}
...
def get_whois_info(domain: str) -> Dict[str, Union[str, List[str], Dict]]:
    whois_info = {}
    try:
...
def print_info(
    ...
    ssl_info: Dict,
    whois_info: Dict,
    ip_geolocation: Optional[Dict],
    open_ports: Union[List[int], str],
...
def main() -> None:
    ...
    parser.add_argument(
        "--waf-info",
        action="store_true",
        help="Check for Web Application Firewall (WAF)",
    )
    ...
    specified_args = any(vars(args).values()) and any(
        [
            args.port_scan,
            args.dns_info,
            args.traceroute,
    ...
    waf_detection, speed_test, js_files, social_links, subdomains = (
        None,
        None,
        None,
        None,
